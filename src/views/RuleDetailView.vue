  ...
  switch (ruleType.value) {
    ...
    case 'qian-shi':
      ruleData.value = {
        ...
        origin: '签诗起源于中国古代的占卜术，最早可以追溯到秦汉时期。它融合了中国传统文化中的诗词、哲学和宗教元素，形成了一种独特的占卜形式。签诗最初与道教、佛教寺庙的占卜活动密切相关，后来逐渐普及到民间，成为中国人日常生活中常见的占卜方式。',
        ...
        rules: '签诗占卜的基本规则包括：<br>• 签诗分为上签、中签、下签三个等级<br>• 每首签诗都有特定的编号和对应的解读<br>• 签诗的内容通常包含典故、比喻和象征<br>• 解读时需要结合问卜者的具体问题<br>• 签诗的吉凶需要辩证看待，凶签也可能包含警示和转机<br>• 同一支签在不同问题下可能有不同的解读',
        interpretation: '签诗的解读需要理解诗歌的字面意思和深层含义，同时结合问卜者的具体问题。本系统会提供签诗的原文、白话文解释以及针对不同问题的建议。<br><br>解读原则包括：<br>• 注意签的等级（上中下）<br>• 理解诗歌中的比喻和象征<br>• 结合具体问题进行灵活解读<br>• 重视行善积德的劝诫<br>• 辩证看待吉凶，关注转机和建议'
      }
      break
    case 'plum-blossom':
      ruleData.value = {
        ...
        interpretation: '梅花易数的解读注重卦象的体用关系、五行生克以及卦气旺衰等因素。本系统会根据生成的卦象，提供详细的卦象分析和运势预测。<br><br>解读原则包括：<br>• 确定体卦和用卦的关系<br>• 分析五行的生克变化<br>• 结合季节和方位因素<br>• 关注动爻的作用和影响<br>• 综合考虑本卦、互卦和变卦的信息<br>• 重视直觉和第一印象'
      }
      break
    default:
      // 默认为易经六十四卦
...
.rule-detail {
  padding: 2rem;
  min-height: 100vh;
  background: linear-gradient(135deg, $cyber-dark 0%, $cyber-panel 100%);
  position: relative;
  display: flex;
...
.rule-card {
  background: rgba($cyber-panel, 0.8);
  border: 2px solid $neon-purple;
  border-radius: 15px;
  padding: 2rem;
  box-shadow: 0 0 30px rgba($neon-purple, 0.3);
...
.rule-header h1 {
  font-size: 2.5rem;
  font-weight: 700;
  background: linear-gradient(90deg, $neon-purple, $neon-pink, $neon-blue);
...
.rule-icons h2 {
  ...
  text-shadow: 0 0 8px rgba($neon-blue, 0.5), 0 0 15px rgba($neon-blue, 0.2);
...
.rule-content p {
  ...
  text-shadow: 0 0 5px rgba($neon-blue, 0.3);  // 添加文字阴影效果
  opacity: 0;  // 初始透明度为0
...
.rule-actions {
  ...
  gap: 1rem;
  padding-top: 1.5rem;
  border-top: 1px solid rgba($neon-blue, 0.3);
...
.cyber-button {
  ...
  padding: 12px 30px;
  background: linear-gradient(90deg, $neon-purple, $neon-blue);
  color: $text-dark;
  font-weight: bold;
...
.cyber-button:hover {
  ...
  box-shadow: 0 5px 20px rgba($neon-purple, 0.8);
}
...
.cyber-button.secondary:hover {
  ...
  box-shadow: 0 5px 20px rgba($neon-blue, 0.5);
}
